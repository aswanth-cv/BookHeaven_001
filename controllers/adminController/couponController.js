...
const createCoupon = async (req, res) => {
  try {
    ...
    if (!code || !discountType || !discountValue || !startDate || !expiryDate) {
      return res.status(401).json({ message: 'All required fields must be provided' });
    }
    ...
    if (start >= expiry) {
      return res.status(401).json({ message: 'Expiry date must be after start date' });
    }

    if (discountType === 'percentage' && (discountValue < 0 || discountValue > 100)) {
      return res.status(401).json({ message: 'Percentage discount must be between 0 and 100' });
    }
    if (discountType === 'fixed' && discountValue <= 0) {
      return res.status(401).json({ message: 'Fixed discount must be greater than 0' });
    }
